...
// Bebas Neue Pro, in true lowercase (the Pro cut has real lowercase; the free
// Bebas is caps-only). A tall condensed display face — a different lane from
// the grotesk finalists, and near recorded ground: BRAND-PRIORS rejected a
// Bebas ALL-CAPS wordmark swap, so this lowercase read is adjacent, not the
// same. Weights are capped by the kit: regular tops at 600, the wider cuts at
// 700 — so it cannot reach the finalists' 900.
const FACES = [
  {
    id: 'bebas',
    label: 'Bebas Neue Pro 600 (regular / condensed — heaviest at this width)',
  ...
  {
    id: 'bebas-semi',
    label: 'Bebas Neue Pro SemiExpanded 700',
    cssFamily: 'bebas-neue-pro-semiexpanded',
    weight: 700,
    kit: true,
    hand: [0, 0, 0, 0.006, -0.03, 0, 0],
  },
];
...
async function main() {
  ...
  writeFileSync(join(OUT, '_look-bebas.html'), sheet(fontFaces));
  await page.goto(pathToFileURL(join(OUT, '_look-bebas.html')).href, { waitUntil: 'networkidle' });
  await page.evaluate(() => document.fonts.ready);

  const h = await page.evaluate(() => document.body.scrollHeight);
  await page.setViewportSize({ width: 1600, height: h });
  await page.screenshot({ path: join(OUT, 'look-bebas.png') });
  await browser.close();
  console.log(`✓ look-bebas.png  1600x${h}`);
}
...
<h1>flickday — Bebas Neue Pro (lowercase)</h1>
<div class="sub">
  Bebas Neue Pro in true lowercase &mdash; the Pro cut has real lowercase; the free Bebas is caps-only.
  A tall condensed display face, a different lane from the grotesk finalists. Note the record: a Bebas
  <em>all-caps</em> wordmark was tried and rejected (BRAND-PRIORS), so this lowercase read is adjacent, not that.
  Weights are capped by the kit &mdash; regular tops at 600, wider cuts at 700 &mdash; so it cannot reach the
  finalists' 900. Same real sizes and by-hand kd as the finish sheet.
</div>
${FACES.map(card).join('')}
</body></html>`;
}
