/**
 * Wordmark finalists (from round 3) — five, real size, native kerning.
 *
 * Round 3's cuts left eight; this narrows to five on standing rather than
 * taste, keeping both ends of the straddle so the final call resolves it:
 *
 *   character — Schibsted 900 (round-2 pick), Roc normal (the width-test
 *               survivor, after the wide cut was blind-rejected), Apotek 700
 *               (the round-3 discovery)
 *   neutral   — Sora 800 and Avenir Next 900 (the two with prior standing)
 *
 * Reshuffled and blind so the round-3 code map does not carry over. Same
 * machinery, same verified real-size normalisation. Native kerning still —
 * the kd-by-hand correction is the finish step for whichever face wins, not
 * something to spend across five.
 *
 *   FLICKDAY_SHUFFLE=1 node scripts/wordmark-finalists-r3.mjs
 *     → flickday-assets/wordmarks/finalists-r3.png
 *     → flickday-assets/wordmarks/finalists-r3-KEY.json   (do not read while judging)
 */
...
const CANDIDATES = [
  { family: 'Schibsted Grotesk', weight: 900, end: 'character', note: 'round-2 pick' },
  {
    ...
    note: 'Roc Grotesk normal — survived after the wide cut was blind-rejected',
  },
  {
    family: 'apotek',
    weight: 700,
    ...
    note: 'Apotek 700 — the round-3 discovery',
  },
  { family: 'Sora', weight: 800, end: 'neutral', note: 'round-1 baseline · round-2 pick' },
  {
    family: 'Avenir Next',
    weight: 900,
    ...
    note: 'documented modular-set base',
  },
];
...
// Five finalists; run with FLICKDAY_SHUFFLE=1 to draw a fresh
// order that neither reviewer has seen.
const FIXED_ORDER = [3, 0, 4, 1, 2];
...
async function main() {
  ...
  const tmp = join(OUT, '_finalists-r3.html');
  ...
  await page.screenshot({ path: join(OUT, 'finalists-r3.png') });
  await browser.close();
  console.log(`✓ finalists-r3.png  1400x${h}`);

  writeFileSync(
    join(OUT, 'finalists-r3-KEY.json'),
    JSON.stringify(
      {
  ...
  console.log('✓ finalists-r3-KEY.json (sealed)');
}
...
<h1>flickday — wordmark finalists (blind, native kerning)</h1>
<div class="sub">
  Five finalists, reshuffled so the round-3 codes do not carry over. Three character grotesks and two neutral
  geometrics — the straddle is meant to resolve here. Which row is which end is in the sealed key.<br>
  <strong>Native kerning, no optical correction.</strong> kd runs open and ck runs tight in most of these; the
  by-hand fix is the finish step for whichever face wins, not something applied across five. Note it, do not judge on it.
</div>
<div class="warn">
